...
const FILTERS: FilterConfig = {
    fields: [
        { name: 'role', type: 'select', placeholder: 'Role', options: ROLE_OPTIONS },
        { name: 'is_active', type: 'select', placeholder: 'Status', options: ACTIVE_OPTIONS },
    ],
...
function UserManagementInner() {
    ...
    const { data: response, isLoading: isFetching, refetch } = useQuery<UsersResponse>({
        queryKey: [ENDPOINTS.ADMIN.USERS, page, pageSize, search, role, isActive, sortBy, sortOrder],
        queryFn: async () => {
    ...
    return (
        <div className="space-y-8">
            <PageHeader
                ...
                title={<>Identity <span className="text-orange-600">Control</span></>}
                actions={
                    <Button
                        variant="outline"
                        onClick={() => refetch()}
