...
interface FooterProps {
  lang: Lang;
  t: any; // The footer slice of your dictionary
}
  ...
  return (
    <footer className="w-full bg-[#18181b] text-white">
      <div className="mx-auto container px-4">
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3 md:gap-8 py-16 gap-y-8">
          
          {/* Brand Section */}
          <div className="col-span-full mb-10 lg:col-span-2 lg:mb-0">
            <Link href={`/${lang}`} className="flex justify-center lg:justify-start items-center gap-2">
              <div className="w-10 h-10 bg-primary rounded-lg flex items-center justify-center font-bold text-xl text-white">E</div>
              <span className="text-2xl font-bold tracking-tight">Cresca</span>
            </Link>
            <p className="py-8 text-sm text-zinc-400 lg:max-w-xs text-center lg:text-left leading-relaxed">
              {t.mission}
            </p>
            
            {/* Language Selector Indicator */}
            <div className="flex items-center justify-center lg:justify-start gap-2 text-zinc-400 mb-6 group cursor-pointer hover:text-white transition-colors">
              <Globe size={16} />
              <span className="text-xs font-medium underline underline-offset-4">
                {t.changeLang} ({langNames[lang]})
              </span>
            </div>

            <Link 
              href={`/${lang}/contact`} 
              className="py-3 px-6 h-11 flex items-center justify-center w-fit bg-primary rounded-full shadow-lg text-sm font-semibold text-white mx-auto transition-all duration-300 hover:bg-primary/90 lg:mx-0"
            >
              {t.contactBtn}
            </Link>
          </div>
          ...
          {t.sections.map((section: any, idx: number) => (
            <div key={idx} className="lg:mx-auto text-left ">
              <h4 className="text-sm font-bold uppercase tracking-widest text-zinc-200 mb-7">
                {section.title}
              </h4>
              <ul className="text-sm space-y-4 transition-all duration-500">
        ...
        <div className="py-8 border-t border-zinc-800">
          <div className="flex items-center justify-center flex-col lg:justify-between lg:flex-row gap-4">
            <span className="text-xs text-zinc-500 text-center lg:text-left">
              © 2026 GlobalEnglish. {t.learners} {t.rights}
            </span>
...
const SocialLink = ({ children, href }: { children: React.ReactNode, href: string }) => (
    <a 
        href={href} 
        className="w-9 h-9 rounded-full bg-zinc-800 flex justify-center items-center hover:bg-primary transition-colors text-white"
    >
        {children}
    </a>
);
